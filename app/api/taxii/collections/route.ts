...
export async function POST(request: NextRequest) {
  try {
    const { apiRootUrl, credentials } = await request.json()

    if (!apiRootUrl) {
      return NextResponse.json({ message: "API Root URL is required" }, { status: 400 })
    }

    // Create the collections URL
    const collectionsUrl = new URL("collections/", apiRootUrl).toString()
